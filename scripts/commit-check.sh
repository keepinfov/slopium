...
# wrapped at eighty columns in no more than two paragraphs, the absence of a
# trailer of any kind — and, for a commit rather than a bare message, the rule
# that a `feat` bumps the workspace version in its own commit and that nothing
# else does.
#
...
check_subject() {
  ...
  if [ -n "$scope" ]; then
    listed "$scope" "$scopes" ||
      problem "unknown scope \`$scope\`; this history uses $scopes, and a new one is a question for the user"
  fi
...
check_message() {
  ...
  if [ "${#body[@]}" -eq 0 ]; then
    if listed "$kind" "$needs_body"; then
...
check_commit() {
  ...
  if [ "$kind" = "feat" ] && [ "$bumped" -eq 0 ]; then
    problem "a \`feat\` is a release: bump \`workspace.package.version\` in this commit"
  fi
  if [ -n "$kind" ] && [ "$kind" != "feat" ] && [ "$bumped" -eq 1 ]; then
    problem "a \`$kind\` commit does not move the version; only a \`feat\` does"
  fi

  if [ "$bumped" -eq 1 ]; then
    printf '%s\n' "$touched" | grep -qx 'Cargo.lock' ||
      problem "the version moved to $new but \`Cargo.lock\` did not follow"
