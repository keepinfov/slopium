...
Ничто в ней не аварийно завершает программу: конец ввода, нечисловая строка,
отсутствующий аргумент или переменная — это `None`, а отказ файловой операции —
`Err` с `errno` (`D-087`). Пакет объявляет `std = { toolchain = true }`
в манифесте; одиночный файл получает библиотеку сам, `--no-std` её отключает,
