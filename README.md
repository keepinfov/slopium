...
начинается сборка; библиотека его не имеет, и отсутствие ключа — это и есть
способ сказать, что пакет библиотечный (то же самое, что `entry = "src/lib.slp"`).
...
`checksum` есть у пакета, чьи байты не могут измениться под lock: сейчас это
только библиотека, входящая в компилятор. У path-зависимости его нет — это
рабочий каталог, и хеширование переписывало бы lock на каждое нажатие клавиши.
