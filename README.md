...
slopium fix                   # переписать устаревшее написание в текущее
slopium fix --check           # показать, что переписал бы fix, без записи
...
`slopium fix` переписывает программу, которую принимал старый toolchain, в
текущее написание. Первое правило — перенос v0.5.1: `print`, `println` и их
соседи перестали быть встроенными и стали именами библиотеки, и `fix`
вставляет недостающие `(take std:io …)` / `(take std:process …)` там, где имя
используется голым и ничем не определено. Файл переписывается целиком или не
трогается вовсе — с сообщением, что и почему исправить нельзя; комментарии и
расположение сохраняются, `fmt` после `fix` ничего не меняет, и повторный
запуск ничего не пишет (`D-160`).
...
В `bench/` лежит набор из семи одинаковых кернелов на пяти языках — Slopium,
C, Rust, Python и Java, — с раннером, который собирает их, проверяет, что
вывод всех пяти совпадает, и меряет время, память, размер артефакта и время
компиляции. Запуск на NixOS:

```sh
nix develop .#bench -c python3 bench/run.py
```

По снапшоту прогона: ~2 мс старта и ~2 МБ памяти на процесс с 18-КБ
бинарем, аллокации вровень с C и Rust, арифметика в 2–4× от `cc -O2` и
впереди Python и JVM почти на всех кернелах — исключение `hash-map`, где
rehash `std:map` растёт квадратично. Методология, правила честности и
снапшот результатов одного прогона — в
[`bench/README.md`](bench/README.md). Ничего из этого не входит в проверочный
набор: время на стене — не то, что репозиторий может обещать.
...
Выйти из функции досрочно — `(return expression)`: выражение типизируется по
результату функции, а сама форма согласуется с тем, чего ждёт место вокруг
неё, потому что выхода не бывает «в позиции значения» — выход и есть
результат. Поэтому `return` может стоять в теле цикла, ожидающем `unit`, и в
одной ветке `if`, когда значение несёт другая. Значение можно вынести через
`return` из ветки-условия: на пути, который ответил, имя ещё владеет своим
значением, и код после ветки им пользуется. Внутри `lambda` `return` выходит
из самой `lambda`, а не из функции, в которой она написана, — как `break`
выходит из ближайшего цикла. Одна форма, без короткого написания: функция,
отвечающая `unit`, пишет `(return ())` (`D-163`).
...
она ни закончилась: концом тела функции, `break`, `continue`, `return` или
ошибочной веткой `try`.
